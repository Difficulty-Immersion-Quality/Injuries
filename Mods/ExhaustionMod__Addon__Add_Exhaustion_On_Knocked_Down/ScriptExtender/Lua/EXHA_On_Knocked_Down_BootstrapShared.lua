...
local function EXHA_Addon_OnKnockedDown_StatsLoaded()
    for _, name in pairs(Ext.Stats.GetStats("Character")) do
        local char = Ext.Stats.Get(name)

        
        if EXHA_CharIsHero(char, name) then
            if string.find(char.Passives, EXHA_Passive_On_Knocked_Down) then
...
Ext.Events.StatsLoaded:Subscribe(EXHA_Addon_OnKnockedDown_StatsLoaded)
