...
local function EXHA_Addon_CustomType_StatsLoaded()
    for _, name in pairs(Ext.Stats.GetStats("Character")) do
        local char = Ext.Stats.Get(name)

        
        if EXHA_CharIsHero(char) then
            if string.find(char.Passives, EXHA_Passive_Custom_Type) then
...
Ext.Events.StatsLoaded:Subscribe(EXHA_Addon_CustomType_StatsLoaded)
